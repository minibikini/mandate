defmodule MandateExamples.BuildTest do
  ...
  test "parses target and switches" do
    assert {:ok, parsed} =
             Mandate.OptionParser.parse(
               ["web", "-r", "--arch", "arm64", "-o", "-o"],
               Mandate.Info.task(Build)
             )
  ...
  test "handles count type switch" do
    assert {:ok, %{optimize: 3}} =
             Mandate.OptionParser.parse(["web", "-o", "-o", "-o"], Mandate.Info.task(Build))
  end

  test "target is optional" do
    assert {:ok, _} = Mandate.OptionParser.parse([], Mandate.Info.task(Build))
  end
end
